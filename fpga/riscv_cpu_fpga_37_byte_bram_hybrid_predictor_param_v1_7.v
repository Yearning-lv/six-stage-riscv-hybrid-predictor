...
module btb #(
    ...
    reg [TAG_BITS-1:0] tags   [0:ENTRY_COUNT-1];
    reg [31:0] targets[0:ENTRY_COUNT-1];
    reg        returns[0:ENTRY_COUNT-1];
    integer i;

    wire [INDEX_BITS-1:0] query_index = query_pc[INDEX_BITS+1:2];
    wire [INDEX_BITS-1:0] update_index = update_pc[INDEX_BITS+1:2];
    wire [TAG_BITS-1:0] query_tag = query_pc[31:INDEX_BITS+2];
    ...
    always @(posedge clk) begin
        if (rst) begin
            ...
            returns[update_index] <= update_is_return;
        end
    end
endmodule
...
module gshare_predictor #(
    ...
    input [31:0] query_pc,
    output       predict_taken,
    output [GHR_BITS-1:0] ghr_value,
    output [INDEX_BITS-1:0] query_index,
    input        update_en,
    ...
    wire [INDEX_BITS-1:0] pc_index = query_pc[INDEX_BITS+1:2];
    wire [INDEX_BITS-1:0] ghr_index;

    assign ghr_value = ghr;
    generate
        if (GHR_BITS >= INDEX_BITS) begin : gen_ghr_truncate
            assign ghr_index = ghr[INDEX_BITS-1:0];
        end else begin : gen_ghr_extend
            assign ghr_index = {{(INDEX_BITS-GHR_BITS){1'b0}}, ghr};
        end
    endgenerate
    assign query_index = pc_index ^ ghr_index;
...
module hybrid_selector #(
    parameter INDEX_BITS = 4
)(
    input       clk,
    input       rst,
    input [31:0] query_pc,
    output      choose_gshare,
    output [INDEX_BITS-1:0] query_index,
    input       update_en,
    input [INDEX_BITS-1:0] update_index,
    ...
    assign choose_gshare = counters[query_index][1];

    always @(posedge clk) begin
        if (rst) begin
            for (i = 0; i < ENTRY_COUNT; i = i + 1)
                counters[i] <= 2'b01;
        end else if (update_en && (pht_pred_taken != gshare_pred_taken)) begin
            if ((gshare_pred_taken == actual_taken) && (counters[update_index] != 2'b11))
                counters[update_index] <= counters[update_index] + 2'b01;
            else if ((pht_pred_taken == actual_taken) && (counters[update_index] != 2'b00))
                counters[update_index] <= counters[update_index] - 2'b01;
        end
    end
endmodule
...
module ras #(
    ...
    localparam SP_BITS = (DEPTH <= 2) ? 2 : $clog2(DEPTH + 1);
    localparam STACK_INDEX_BITS = (DEPTH <= 2) ? 1 : $clog2(DEPTH);
    ...
    always @(posedge clk) begin
        if (rst) begin
            ...
            if (push_en && (sp < DEPTH)) begin
                stack[sp[STACK_INDEX_BITS-1:0]] <= push_data;
                sp <= sp + 1'b1;
...
module pipeline_cpu_top #(
    ...
    wire        pht_predict_taken_f;
    wire        gshare_predict_taken_f;
    wire [GHR_BITS-1:0] gshare_ghr_f;
    wire [PRED_INDEX_BITS-1:0] gshare_index_f;
    wire        selector_choose_gshare_f;
    wire [PRED_INDEX_BITS-1:0] selector_index_f;
    ...
    reg         pht_pred_fetch_q;
    reg         gshare_pred_fetch_q;
    reg  [PRED_INDEX_BITS-1:0] gshare_index_fetch_q;
    reg  [PRED_INDEX_BITS-1:0] selector_index_fetch_q;
    reg         fetch_valid_q;
    reg         fetch_flush_q;
    reg         stall_q;
    ...
    reg         stall_pht_pred_q;
    reg         stall_gshare_pred_q;
    reg  [PRED_INDEX_BITS-1:0] stall_gshare_index_q;
    reg  [PRED_INDEX_BITS-1:0] stall_selector_index_q;
    reg         stall_valid_q;
    ...
    reg        if_id_pht_pred_taken;
    reg        if_id_gshare_pred_taken;
    reg [PRED_INDEX_BITS-1:0] if_id_gshare_index;
    reg [PRED_INDEX_BITS-1:0] if_id_selector_index;
    reg        if_id_valid;
    ...
    reg        id_ex_pht_pred_taken;
    reg        id_ex_gshare_pred_taken;
    reg [PRED_INDEX_BITS-1:0] id_ex_gshare_index;
    reg [PRED_INDEX_BITS-1:0] id_ex_selector_index;
    reg        id_ex_valid;
    ...
    assign dir_pred_taken_f = btb_hit_f &&
                              (selector_choose_gshare_f ? gshare_predict_taken_f :
                                                          pht_predict_taken_f);
    ...
    pht #(
        ...
        .rst(rst),
        .query_pc(pc_f),
        .predict_taken(pht_predict_taken_f),
        .update_en(btb_update_en),
        .update_pc(btb_update_pc),
    ...
    hybrid_selector #(
        ...
        .rst(rst),
        .query_pc(pc_f),
        .choose_gshare(selector_choose_gshare_f),
        .query_index(selector_index_f),
        .update_en(btb_update_en),
        .update_index(id_ex_selector_index),
        .pht_pred_taken(id_ex_pht_pred_taken),
        .gshare_pred_taken(id_ex_gshare_pred_taken),
        .actual_taken(ex_control_taken)
    );
    ...
    always @(posedge clk) begin
        if (rst) begin
            ...
            pht_pred_fetch_q <= 1'b0;
            gshare_pred_fetch_q <= 1'b0;
            gshare_index_fetch_q <= {PRED_INDEX_BITS{1'b0}};
            selector_index_fetch_q <= {PRED_INDEX_BITS{1'b0}};
            fetch_valid_q <= 1'b0;
            fetch_flush_q <= 1'b0;
            stall_q       <= 1'b0;
            stall_instr_q <= 32'b0;
            ...
            stall_pht_pred_q <= 1'b0;
            stall_gshare_pred_q <= 1'b0;
            stall_gshare_index_q <= {PRED_INDEX_BITS{1'b0}};
            stall_selector_index_q <= {PRED_INDEX_BITS{1'b0}};
            stall_valid_q <= 1'b0;
        end else begin
            stall_q <= stall;
            ...
            if (!stall) begin
                ...
                pht_pred_fetch_q <= pht_predict_taken_f;
                gshare_pred_fetch_q <= gshare_predict_taken_f;
                gshare_index_fetch_q <= gshare_index_f;
                selector_index_fetch_q <= selector_index_f;
                fetch_valid_q <= 1'b1;
            end
            ...
            if (stall && !stall_q) begin
                ...
                stall_pht_pred_q <= pht_pred_fetch_q;
                stall_gshare_pred_q <= gshare_pred_fetch_q;
                stall_gshare_index_q <= gshare_index_fetch_q;
                stall_selector_index_q <= selector_index_fetch_q;
    ...
    always @(posedge clk) begin
        if (rst) begin
            ...
            if_id_pht_pred_taken <= 1'b0;
            if_id_gshare_pred_taken <= 1'b0;
            if_id_gshare_index <= {PRED_INDEX_BITS{1'b0}};
            if_id_selector_index <= {PRED_INDEX_BITS{1'b0}};
            if_id_valid <= 1'b0;
            ...
            if_id_pht_pred_taken <= if_id_pht_pred_taken;
            if_id_gshare_pred_taken <= if_id_gshare_pred_taken;
            if_id_gshare_index <= if_id_gshare_index;
            if_id_selector_index <= if_id_selector_index;
            ...
            if_id_pht_pred_taken <= stall_pht_pred_q;
            if_id_gshare_pred_taken <= stall_gshare_pred_q;
            if_id_gshare_index <= stall_gshare_index_q;
            if_id_selector_index <= stall_selector_index_q;
            ...
            if_id_pht_pred_taken <= pht_pred_fetch_q;
            if_id_gshare_pred_taken <= gshare_pred_fetch_q;
            if_id_gshare_index <= gshare_index_fetch_q;
            if_id_selector_index <= selector_index_fetch_q;
    ...
    always @(posedge clk) begin
        if (rst) begin
            ...
            id_ex_pht_pred_taken <= 1'b0;
            id_ex_gshare_pred_taken <= 1'b0;
            id_ex_gshare_index <= {PRED_INDEX_BITS{1'b0}};
            id_ex_selector_index <= {PRED_INDEX_BITS{1'b0}};
            id_ex_valid      <= 1'b0;
        end else if (ex_mispredict) begin
            id_ex_pc4      <= 32'b0;
            id_ex_rdata1   <= 32'b0;
            ...
            id_ex_pht_pred_taken <= if_id_pht_pred_taken;
            id_ex_gshare_pred_taken <= if_id_gshare_pred_taken;
            id_ex_gshare_index <= if_id_gshare_index;
            id_ex_selector_index <= if_id_selector_index;
