...
$configs = @(
    @{ Id = 'R2'; Entries = 32; IndexBits = 5; GhrBits = 5; RasDepth = 2 },
    @{ Id = 'R3'; Entries = 32; IndexBits = 5; GhrBits = 5; RasDepth = 4 },
    @{ Id = 'R4'; Entries = 32; IndexBits = 5; GhrBits = 5; RasDepth = 8 }
)
...
$benchmarks = @(
    ...
    @{
        Name = 'call_return_long'
        Hex = 'tests\benchmarks\call_return_long.hex'
        End = 15
        Args = @('+EXP_X10=10', '+EXP_X11=10', '+EXP_X12=80')
    },
    @{
        Name = 'call_return_nested'
        Hex = 'tests\benchmarks\call_return_nested.hex'
        End = 35
        Args = @('+EXP_X2=64', '+EXP_X10=3', '+EXP_X11=3', '+EXP_X12=3', '+EXP_X13=3', '+EXP_X14=3', '+EXP_X15=18')
    },
    @{
        Name = 'memory_loop'
...
foreach ($config in $configs) {
    $outFile = Join-Path $outDir ("v17_ras_" + $config.Id + ".vvp")
...
$csv = Join-Path $resultDir 'v1_7_ras_sweep_results.csv'
...
if ($totalFail -eq 0) {
    Write-Output "V1.7 RAS SWEEP PASS"
    exit 0
} else {
    Write-Output "V1.7 RAS SWEEP FAIL: $totalFail failure(s)"
    exit 1
}
