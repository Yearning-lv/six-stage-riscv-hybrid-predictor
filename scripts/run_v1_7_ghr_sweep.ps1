...
$configs = @(
    @{ Id = 'G1'; Entries = 32; IndexBits = 5; GhrBits = 2; RasDepth = 4 },
    @{ Id = 'G2'; Entries = 32; IndexBits = 5; GhrBits = 4; RasDepth = 4 },
    @{ Id = 'G3'; Entries = 32; IndexBits = 5; GhrBits = 6; RasDepth = 4 },
    @{ Id = 'G4'; Entries = 32; IndexBits = 5; GhrBits = 8; RasDepth = 4 }
)
...
foreach ($config in $configs) {
    $outFile = Join-Path $outDir ("v17_ghr_" + $config.Id + ".vvp")
...
$csv = Join-Path $resultDir 'v1_7_ghr_sweep_results.csv'
...
if ($totalFail -eq 0) {
    Write-Output "V1.7 GHR SWEEP PASS"
    exit 0
} else {
    Write-Output "V1.7 GHR SWEEP FAIL: $totalFail failure(s)"
    exit 1
}
